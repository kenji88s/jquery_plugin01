(function ($) {
    ...
    var defaults = {
        ...
        loadingSelector: '*[class^=".fulfilling-"]',
        loadingImage: 'none',
        loadingImageSrc: 'img/works-img08.jpg',
        ...
        /*
        //ページ遷移時にモーダル
        readPlay: 'none', // none（無し）,timeInterval（時間間隔）,count（〇回毎に）,every（毎回）
        //ページ遷移時にモーダル（時間間隔）を選択した場合、再度出現させたいときの分数
        readIntervalTime: 60,
        ...
        //youTube動画のパラメータ
        youtubeParm: '?rel=0&autoplay=1&mute=1'
    ...
    $.fn.ymodal = function (options, num = null) {
        ...
        settings.finishLoading = function () {
            $('.modal_window_box iframe').load(function () {
                setTimeout(function () {
                    (settings.deleteLoading() && settings.deleteThumb() ? clearTimeout() : null);
                }, 1000);
            });
            return true;
        };
        ...
        var storage_action = {
            ...
            keyName: function () {
                ...
                /* ↑ readPlay: timeInterval（時間間隔）,count（〇回毎に）,every（毎回）のとき1ページに1個まで */
                ...
                if (sessionStorage.getItem(`single-${target}`) == null && settings.readPlay != 'none') {
                    ...
                    sessionStorage.setItem(`single-${target}`, true);
                };
                /* ↑ セッションストレージにキー名が存在しないかつreadPlay: none（無し）以外のときキー名を作成 */

                if (settings.readPlay == 'none' && settings.storage) {
                    settings.storageKey = `${element.selector}-${target}`;
                };
                /* ↑ readPlay: none（無し）かつstorage: true（セッションストレージの使用）のときキー名を作成 */

                if (sessionStorage.getItem(`single-${target}`) != null && settings.readPlay != 'none') {
                    settings.readPlay == 'none';
                    settings.storage == false;
                };
                /* ↑ セッションストレージにキー名が存在かつreadPlay: none（無し）以外のとき無効化 */
            ...
            makeUrl: function () {
                ...
                settings.youtubeParm != '' ? url += settings.youtubeParm : null;
                url = url.replace('&autoplay=1', '').replace('&autoplay=0', '');
                settings.playAuto ? url += '&autoplay=1' : null;
                return url;
            },
        ...
        $(function () {
            ...
            if (storage_action.keyName() && storage_action.remove() && storage_action.playInfo() && storage_action.modalSource()) {
                return false;
            };
            /* ↑ すべてのストレージのアクションの実行 */
        });
